...
const AllUsers = () => {
    ...
    const handleMakeAdmin = user => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axiosSecure.patch(`/user/admin/${user.email}`)
                    .then(res => {
                        refetch();
    ...
    const handleDelete = user => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:4321/users/${user._id}`)
                    .then(res => {
                        ...
                        <tbody>
                            {
                                users.map((user, index) => <tr key={user._id}>
                                    ...
                                    <td>
                                        {
                                            user.role === 'admin' ? 'Admin' : <FaUserGraduate onClick={() => handleMakeAdmin(user)} className='text-4xl bg-[#ac7e13af] text-white p-2 cursor-pointer rounded-md'></FaUserGraduate>
                                        }
                                    </td>
                                    <td>
                                        <MdDelete onClick={() => handleDelete(user)} className='text-4xl cursor-pointer bg-red-700 text-white p-2 rounded-md'></MdDelete>
                                    </td>
                                </tr>)
                            }
